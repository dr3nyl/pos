<x-app-layout>
    <x-slot name="header">
        <h2 class="flex justify-center font-semibold text-2xl text-gray-800 leading-tight">
            {{ __('Profile') }}
        </h2>
    </x-slot>

    <x-card outerDivAttribute="pt-8" innerDivAttribute="w-3/5 sm:px-6 lg:px-8 p-6">
        <div class="flex items-center mb-4">
            <img class="rounded-full mr-2 hover:border hover:border-gray-100" src="{{ asset('storage/' . auth()->user()->photo)}}" alt="" width="130" height="130">
            <div class="flex flex-col ml-2 w-full">
                <!-- Post name -->
                <h2 class="text-2xl font-semibold font-sans">Paimon Spacedog</h2>
                <!-- Date posted -->
                <p class="text-md text-gray-400">43 followers</p>

                
                <div class="ml-auto mt-2">
                    <x-button>Follow</x-button>
                </div>
                
            </div>
        </div>
    </x-card>

    <div class="flex items-center justify-center">

            <div class="grid grid-rows-7 grid-cols-4 gap-3 w-3/5 sm:px-6 lg:px-0 pt-6">
                <div class="row-span-2 col-span-2 bg-white border-b border-gray-200 rounded-lg shadow-lg">
                  <!-- Write something -->
                </div>
                
                <div class="col-span-2 bg-white border-b border-gray-200 rounded-lg shadow-lg">
                    <x-card outerDivAttribute="" innerDivAttribute="sm:px-6 lg:px-8 p-6">
                        <form wire:submit.prevent="store" action="" method="post">
                            <div class="header flex items-center">
                                <img src="" alt="">
                                <h2>{{ auth()->user()->name }}</h2>
                            </div>
                            <div class="body">
                                <textarea wire:model.defer="body" class="rounded-lg mt-3 mb-3 w-full border-gray-200" name="post" id="post" cols="54" rows="2" placeholder="Say something.." required></textarea>

                            </div>
                            <div class="footer">
                                <x-button 
                                    class="">
                                    <svg 
                                        wire:loading 
                                        wire:click.prevent="store" 
                                        role="status" 
                                        class="w-4 h-5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" 
                                        viewBox="0 0 100 101" 
                                        fill="none" 
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"></path>
                                        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"></path>
                                    </svg>

                                    Post
                                </x-button>
                            </div>
                        </form>
                    </x-card>
                </div>
                
                <div class="row-span-3 col-span-2  rounded-lg shadow-lg">
                    <!-- Display wall post -->
                    <livewire:post-section :isUserInProfileSection="1"/>
                </div>
            </div>
     
    </div>
</x-app-layout>