...
<button @click=
    ...
    ">
        ...
        @if ($notificationCount)
            <div class="absolute rounded-full bg-red-500 text-white text-xxs w-5 h-5 flex justify-center items-center border-2 -top-1 -right-1">{{ $notificationCount }}</div>
        @endif
    </button>
    <ul
        class="absolute w-76 md:w-96 text-left text-gray-700 text-sm bg-gray-100 shadow-md rounded-xl max-h-128 overflow-y-auto z-10 -right-28 md:-right-12"
        x-cloak
    ...
    >
        ...
        @if ($notifications->isNotEmpty() && ! $isLoading)
            ...
            <li>
                ...
                >
                    <img src="{{ asset('storage/' . $notification->data['user_avatar']) }}" class="rounded-xl w-10 h-10" alt="avatar">
                    <div class="ml-4">
                        <div class="line-clamp-6">
                            <span class="font-semibold">{{ $notification->data['user_name'] }}</span> commented on
                            <span class="font-semibold">{{ $notification->data['post_body'] }}</span>:
